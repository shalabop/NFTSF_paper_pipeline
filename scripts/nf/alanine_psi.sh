...
#SBATCH --gres=gpu:2
#SBATCH --mem=32G
#SBATCH --output=logs/nf/psi.%j.out
#SBATCH --error=logs/nf/psi.%j.err
...
python eval/NF/forecast_nf.py \
    --model_path   checkpoints/nf/alanine_psi.pth     \
    --config configs/nf/alanine_psi.yaml \
    --data_path    DATA/alanine_psi_test.npz    \
    --out          results/nf/alanine_psi.npz \
    --context_length     100    \
