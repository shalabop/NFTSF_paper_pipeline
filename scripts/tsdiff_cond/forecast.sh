...
#SBATCH --mem=32G
#SBATCH --output=logs/tsdiff_cond/evaluation.%j.out
#SBATCH --error=logs/tsdiff_cond/evaluation.%j.err

mkdir -p results/csdi
mkdir -p results/csdi/alanine_phi
#mkdir -p logs/csdi/alanine_phi
mkdir -p checkpoints/tsdiff_cond/
...
python eval/TSDiff/forecast_tsdiff_cond.py   \
        ...
        --dataset_path gluonts_datasets/alanine_psi \
        --out results/tsdiff_cond/alanine_psi_50_50.npz
