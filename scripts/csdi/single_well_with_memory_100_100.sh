#!/usr/bin/env bash
#SBATCH --job-name=SWCSDmemory100100
...
#SBATCH --gres=gpu:1
#SBATCH --mem=16G
#SBATCH --output=logs/csdi/single_well_with_memory_100_100.%j.out
#SBATCH --error=logs/csdi/single_well_with_memory_100_100.%j.err

mkdir -p results/csdi/single_well
#mkdir -p logs/csdi/double_well
mkdir -p checkpoints_100_100/csdi/single_well_with_memory
...
python train/CSDI/train_csdi.py \
    --config configs/csdi_train/single_well_with_memory_100_100.yaml \
    --input  DATA/single_well_with_memory_train.npz \
    --out    checkpoints_100_100/csdi/single_well_with_memory \
    --device cuda:0
