...
#SBATCH --gres=gpu:2
#SBATCH --mem=32G
#SBATCH --output=logs/ratd/single_well.%j.out
#SBATCH --error=logs/ratd/single_well.%j.err

mkdir -p logs/ratd
mkdir -p results/ratd
mkdir -p checkpoints/ratd
mkdir -p checkpoints/tcn
mkdir -p references/single_well
...
python train/RATD/train_tcn.py --config configs/tcn/single_well.yaml

python train/RATD/retrieval_builder.py --config configs/tcn/single_well.yaml --type encode

python train/RATD/retrieval_builder.py --config configs/tcn/single_well.yaml --type retrieval

python train/RATD/train_ratd.py --config configs/ratd/single_well.yaml

python eval/RATD/forecast_ratd.py --config configs/ratd/single_well.yaml
