...
# n_past and n_future for synthetic Langevin datasets (single_well, double_well)
...
# Alanine Dipeptide is real molecular dynamics data — enforce 50/50 split.
ALANINE_N_PAST=50
ALANINE_N_FUTURE=50
...
"${PYTHON_RUNNER[@]}" compare_models.py \
    --nftsf_dirs \
        ...
        "alanine_phi:${ALANINE_PHI_DATA_DIR}:${ALANINE_PHI_TRAIN_DIR}:multi_sim:${ALANINE_N_PAST}:${ALANINE_N_FUTURE}" \
        "alanine_psi:${ALANINE_PSI_DATA_DIR}:${ALANINE_PSI_TRAIN_DIR}:multi_sim:${ALANINE_N_PAST}:${ALANINE_N_FUTURE}" \
