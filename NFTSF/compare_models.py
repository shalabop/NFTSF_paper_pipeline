...
python compare_models.py \\
    --nftsf_data \\
        ...
        alanine_phi:phi_test.npy:phi_model.pth:phi_config.json::tnf:100:50 \\
        alanine_psi:psi_test.npy:psi_model.pth:::tnf:100:50 \\
...
LANDSCAPE_DISPLAY: dict[str, str] = {
    ...
    # SDE landscapes — model_key is used directly as the landscape identifier.
    "sw_sle_em":      "SW SLE (no memory)",
    "sw_gle_oe_em":   "SW GLE (memory)",
    # Double-well SDE keys — PENDING (add integrators and configs first):
    # "dw_sle_em":    "DW SLE (no memory)",
    # "dw_gle_oe_em": "DW GLE (memory)",
}
...
COORDINATE_LABEL: dict[str, str] = {
    ...
    # SDE landscapes share the same position label.
...
def _parse_nftsf_spec(spec: str) -> dict:
    ...
    return {
        ...
        # Optional 9th field: model_variant; None means "read from config or default ar".
        "model_variant": parts[8] if len(parts) > 8 and parts[8] else None,
    }
...
def _parse_nftsf_dirs_spec(spec: str) -> dict:
    ...
    # ---- n_past / n_future / model_variant from config ----
...
def _load_test_data(data_path: str, data_format: str) -> torch.Tensor:
    ...
    Supported formats
    -----------------
    single_sim  : raw shape (Samples, Time, 2), positions at [:, :, 1]
    multi_sim   : raw shape (Time, 1+NumSims), column 0 is time
    tnf         : raw shape (T, N, F), general format
    sde_x       : SDE position file, already (N_traj, T) — no reshaping needed.
                  Pass the *_x.npy file directly (pre-split test partition).
    """
    ...
    elif data_format == "sde_x":
        # SDE position trajectories: shape is already (N_traj, T).
        # Pass the pre-split x_test.npy (or the full x.npy for the comparison
        # script — comparisons must use consistent test partitions).
        return t
    else:
...
def _load_nftsf_model(
    ...
    model_variant resolution order:
      1. Explicit argument (caller-supplied, e.g. from --nftsf_data 9th field).
      2. config JSON 'model_variant' key (loaded here when config_path given).
      3. Default 'ar' for backward compatibility with old checkpoints.
    """
    flow_blocks   = 6
    hidden_units  = 64
    hidden_layers = "1,2"
    ...
    if config_path and Path(config_path).exists():
        ...
        cfg_variant   = cfg.get("model_variant", cfg_variant)

    # Caller-supplied value overrides config; both fall back to 'ar'.
    resolved_variant = model_variant if model_variant is not None else cfg_variant
    print(f"    model_variant={resolved_variant}")

    if resolved_variant == "ar":
    ...
    try:
        ...
        raise RuntimeError(
            f"Checkpoint is incompatible with resolved_variant={resolved_variant!r}, "
            f"n_past={n_past}, n_future={n_future}, model_path={model_path!r}. "
...
def run_arima_full_eval(
    ...
    starts = np.full(n_trajs, max(0, traj_len - n_extrp), dtype=np.int64)
    work   = []
...
def save_evaluation_npz(
    landscape: str,
    model_data: dict,
    output_dir: Path,
) -> Path:
    """
    Save the full-testset evaluation results for one landscape/model pair to
    an .npz file that can be read by compare_methods.py.

    The file is named:  results_{landscape}_{model}.npz
    (model name is stored in model_data["model_name"] — callers must add it)

    NPZ keys written
    ----------------
    samples              : (N_display, n_samples, n_future)  float32
    ground_truth         : (N_display, n_future)             float32  — forecast portion only
    full_trajectories    : (N_display, n_past+n_future)      float32  — context + forecast
    n_past               : scalar int
    prediction_length    : scalar int  (= n_future)
    train_test_split     : scalar int  (= n_past, the index where forecast starts within the window)
    fullset_mae          : (n_future,)  float32  — averaged over all N evaluated trajectories
    fullset_crps         : (n_future,)  float32
    fullset_ci50         : (n_future,)  float32
    fullset_ci90         : (n_future,)  float32
    fullset_N            : scalar int
    """
    model_name = model_data.get("model_name", "unknown")
    n_past   = int(model_data["n_past"])
    n_future = int(model_data["n_future"])

    gts     = model_data["ground_truths"]          # (N_disp, n_past+n_future)
    samples = model_data["samples"]                # (N_disp, n_samples, n_future)

    save_dict = {
        "samples":           samples.astype(np.float32),
        "ground_truth":      gts[:, n_past:].astype(np.float32),   # forecast portion
        "full_trajectories": gts.astype(np.float32),               # full window
        "n_past":            np.int64(n_past),
        "prediction_length": np.int64(n_future),
        "train_test_split":  np.int64(n_past),   # within the saved window, context ends at n_past
    }

    # Include fullset metrics if present (computed over all N trajectories)
    for key in ("fullset_mae", "fullset_crps", "fullset_ci50", "fullset_ci90", "fullset_N"):
        if key in model_data and model_data[key] is not None:
            save_dict[key] = model_data[key]

    out_path = output_dir / f"results_{landscape}_{model_name}.npz"
    np.savez(out_path, **save_dict)
    print(f"  [NPZ] Saved evaluation results: {out_path}")
    return out_path
...
def main() -> None:
    ...
    for sp in specs:
        ...
        model = _load_nftsf_model(
            ...
            model_variant=sp.get("model_variant"),
        )
    ...
    # ---------------------------------- Save NFTSF evaluation results
    print("\n=== Saving NFTSF evaluation results (.npz) ===")
    for land in landscapes:
        nftsf_data[land]["model_name"] = "nftsf"
        save_evaluation_npz(land, nftsf_data[land], out)
    ...
    if args.no_arima:
        ...
        # ---------------------------------- Save ARIMA evaluation results
        print("\n=== Saving ARIMA evaluation results (.npz) ===")
        for land in landscapes:
            arima_data[land]["model_name"] = "arima"
            save_evaluation_npz(land, arima_data[land], out)
