...
def _crps_per_step(gt_future: np.ndarray, samples: np.ndarray) -> np.ndarray:
    N, n_future = gt_future.shape
    n_samp = samples.shape[1]
    crps_t = np.zeros(n_future)
    for t in range(n_future):
        obs = gt_future[:, t]
        fc = samples[:, :, t]
        mae_term = np.mean(np.abs(fc - obs[:, None]), axis=1)
        fc_sorted = np.sort(fc, axis=1)
        diff = fc_sorted[:, 1:] - fc_sorted[:, :-1]
        w = np.arange(1, n_samp) * np.arange(n_samp - 1, 0, -1)
        energy = (diff * w[None, :]).sum(axis=1) / (n_samp ** 2)
...
def plot_metric_tables(landscapes: list[str], model_names: list[str],
    ...
    for landscape in landscapes:
        ...
        for model_name in model_names:
            ...
            table_data.append([f"{all_metrics[landscape][model_name][k].mean():.4f}" for k in METRIC_ORDER])
