...
RATD_DIR = os.path.join(ROOT, "architectures", "RATD")
sys.path.insert(0, RATD_DIR)
...
def forecast_ratd(config):
    ...
    time = data.get('time', np.arange(positions.shape[1]))
    ...
    indices = torch.load(config['path']['ref_path'].replace('.pt', '_test.pt'))
    ...
    ###debug
    samples = samples_out[:, :, -H:]        # (N, n_samples, H)
