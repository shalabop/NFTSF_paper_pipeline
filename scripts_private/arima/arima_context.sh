...
#SBATCH --gres=gpu:2
#SBATCH --mem=32G
#SBATCH --output=logs/arima/arima_all_ct.%j.out
#SBATCH --error=logs/arima/arima_all_ct.%j.err
...
python eval/ARIMA/run_auto_arima.py \
    ...
    --out results/arima/linear_gaussian_fh.npz \
    --model 3000 \
    --num-samples 500 \
    -c configs/arima_fh/linear_gaussian.yaml
...
python eval/ARIMA/run_auto_arima.py \
    ...
    --out results/arima/alanine_psi.npz \
    --model 3000 \
    --num-samples 500 \
