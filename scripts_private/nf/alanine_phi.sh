...
#SBATCH --gres=gpu:2
#SBATCH --mem=32G
#SBATCH --output=logs/nf/double_well.%j.out
#SBATCH --error=logs/nf/double_well.%j.err
...
python eval/NF/forecast_nf.py \
    --model_path   checkpoints/nf/alanine_phi.pth     \
    --config configs/nf/double_well.yaml \
    --data_path    DATA/alanine_phi_test.npz    \
    --out          results/nf/alanine_phi.npz \
    --context_length     100    \
