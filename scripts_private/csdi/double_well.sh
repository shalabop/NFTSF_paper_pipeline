...
#SBATCH --gres=gpu:1
#SBATCH --mem=16G
#SBATCH --output=logs/csdi/double_well_2525_5050.%j.out
#SBATCH --error=logs/csdi/double_well_2525_5050.%j.err

mkdir -p results/csdi/double_well
#mkdir -p logs/csdi/double_well
mkdir -p checkpoints_25_25/csdi/double_well
mkdir -p checkpoints_50_50/csdi/double_well
...
python train/CSDI/train_csdi.py \
    ...
    --input  DATA/double_well_train.npz \
    --out    checkpoints_50_50/csdi/double_well \
    --device cuda:0
