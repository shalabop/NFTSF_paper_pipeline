...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default=torch.device("cuda:3" if torch.cuda.is_available() else "cpu"), help='device')
    ...
    # forecasting task
    parser.add_argument('--cont_len', type=int, default=48, help='context sequence length')
    ...
    parser.add_argument('--cont_hidden_dim', type=int, default=128, help='hidden size of context, options:[128, 256, 512, 768]')
    parser.add_argument('--pred_hidden_dim', type=int, default=128, help='hidden size of predition, options:[128, 256, 512, 768]')
    parser.add_argument('--step_hidden_dim', type=int, default=128, help='hidden size of diffusion step')
    parser.add_argument('--time_hidden_dim', type=int, default=128, help='hidden size of timestamp')
    ...
    # noise schedule
    parser.add_argument('--n_steps', type=int, default=50, help='diffusion steps, options:[50, 100]')  # 50
    parser.add_argument('--beta_start', type=int, default=0.0001, help='beta starting value')
    parser.add_argument('--beta_end', type=int, default=0.5, help='beta end value')  # 0.5
    ...
    if args.status == "train":
        ...
        # # two-stage training
        # model = DiffMTS(configs=args, data_loader=data_loader)
        # model.train(is_refine=True, init_epoch=14, n_epochs=150)
        ...
        model.test(best_epoch=17, n_samples=20, is_out=False)
