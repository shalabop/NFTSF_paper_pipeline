...
class ReferenceModulatedCrossAttention(nn.Module):
    ...
    def forward(
        ...
        #print(f"  reference.shape before repeat: {reference.shape}")
        #print(f"  Expected dim (from init): {self.y_to_q.in_features}")  # Should be L
        #print(f"  Actual L from data: {L}")
        #print(f"  context_dim (from init): {self.cond_to_k.in_features - 2*self.y_to_q.in_features}")
...
class diff_RATD(nn.Module):
    ...
    def forward(self, x, cond_info, diffusion_step, reference=None):
        ...
        #print(f'inputdim = {inputdim}' ) #num_sample_features
        #print(f'K = {K}' )  ##1 for univariate data
        #print(f'L = {L}' ) #L+H
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, cond_info, diffusion_emb, reference):
        ...
        if reference is not None and self.fusion_type==1:
            ...
            ###
            y_4d = y.reshape(B, channel, K, L)           # (B, C, K, L)
            cond_4d = cond_info.reshape(B, channel, K, L) # (B, C, K, L)

            cond_info_rma = self.RMA(y_4d, cond_4d, reference)  # (B*C, K, L)
            cond_info = cond_info_rma.reshape(B, channel, K, L).reshape(B, channel, K*L)
