    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if (pl_module.current_epoch + 1) % self.eval_every == 0:
            ...
            for label, state_dict in zip(
                ...
                evaluator = Evaluator(num_workers=4)
                ...
                #first_entry = next(iter(transformed_valdata))
                #target_length = len(first_entry["target"])
                
                #print(f'trian + validation time steps : {target_length}')
                
                #first_pair = next(iter(self.val_data))
                #train_part = first_pair[0]
                #val_part = first_pair[1]

                #print(f'validation time steps : {len(val_part["target"])}')
                #print(f'train time steps : {len(train_part["target"])}')

                #first_transformed = next(iter(transformed_valdata))
                #print(f'train + validation time steps : {len(first_transformed["target"])}')
                #print("261 FORCAST ###################################################################################")'''

                val_list = list(transformed_valdata)
                n_val = len(val_list)
                ...
                ##################debugging
                forecasts_pytorch = list(tqdm(
                    forecast_it,
                    total=n_val,
                    desc=f"Validation epoch {pl_module.current_epoch + 1}",
                ))
