...
past = ts.values[:config["context_length"] + max(model.lags_seq)]
future_true = ts.values[len(past):]  # Ground-truth future values
...
# Separator line between past and future
...
plt.legend()
plt.ylim(0, 200)
#plt.xlim(3900, 4000 + 200)
plt.xlim(3900, T_past + T_future - 1)
plt.tight_layout()
plt.savefig("forecast_electric2.png", dpi=150)
plt.close()
