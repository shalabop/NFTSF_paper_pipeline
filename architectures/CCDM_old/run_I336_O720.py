...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='CCDM')

    # basic config
    parser.add_argument('--status', type=str, default='test', help='experiment type')
    parser.add_argument('--device', default=torch.device("cuda:6" if torch.cuda.is_available() else "cpu"), help='device')
    ...
    # forecasting task
    parser.add_argument('--cont_len', type=int, default=336, help='context sequence length')
    ...
    parser.add_argument('--use_window_norm', type=bool, default=True, help='use instance-wise normalization and '
                                                                           'denormalization to remove non-stationarity')  # True
    parser.add_argument('--n_emb', type=int, default=2, help='num of input embedding layers')
    parser.add_argument('--cont_hidden_dim', type=int, default=768, help='hidden size of context, options:[128, 256, 512, 768]')
    parser.add_argument('--pred_hidden_dim', type=int, default=768, help='hidden size of predition, options:[128, 256, 512, 768]')
    parser.add_argument('--step_hidden_dim', type=int, default=768, help='hidden size of diffusion step')
    parser.add_argument('--time_hidden_dim', type=int, default=768, help='hidden size of timestamp')
    ...
    # noise schedule
    parser.add_argument('--n_steps', type=int, default=200, help='diffusion steps, options:[50, 100]')
    parser.add_argument('--beta_start', type=int, default=0.0001, help='beta starting value')
    parser.add_argument('--beta_end', type=int, default=0.1, help='beta end value')
    ...
    if args.status == "train":
        ...
        args.use_contrast = 'contrast'
        args.contrast_weight = 0.00005
        args.test_batch_size = 1
        data_loader = create_mts_loader(args)
        model = DiffMTS(configs=args, data_loader=data_loader)

        model.test(best_epoch=24, n_samples=20, is_out=False)
