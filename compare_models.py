...
def parse_args() -> argparse.Namespace:
    ...
    help=(
        "Path to _test.npz file containing mean/std for denormalization. "
        "Format: landscape:path/to/ds_test.npz  "
        "e.g. double_well:DATA/double_well_test.npz"
    ),
)
    return p.parse_args()

def _load_mean_std(data_npz_specs: list[str] | None) -> dict[str, tuple[float, float]]:
    """
    Parse --data_npz entries and return {landscape: (mean, std)}.
    If not provided, returns empty dict (no denormalization).
    """
    result = {}
    if not data_npz_specs:
        return result
    for spec in data_npz_specs:
        parts = spec.split(":")
        if len(parts) != 2:
            print(f"WARNING: --data_npz entry malformed (expected landscape:path): {spec!r}")
            continue
        land, path = parts
        ...
        print(f"  [{land}] loaded norm stats: mean={mean:.4f}  std={std:.4f}")
    return result
...
def main() -> None:
    ...
    norm_stats_map = _load_mean_std(args.data_npz)

    print("\n=== Denormalizing results ===")
    for land in landscapes:
        if land not in norm_stats_map:
            print(f"  [{land}] no norm stats found — skipping denormalization")
            continue
        mean, std = norm_stats_map[land]
        print(f"  [{land}] denormalizing with mean={mean:.4f}  std={std:.4f}")

        nftsf_data[land]["ground_truths"] = _denorm(nftsf_data[land]["ground_truths"], mean, std)
        nftsf_data[land]["samples"]       = _denorm(nftsf_data[land]["samples"],       mean, std)

        if land in arima_data:
            arima_data[land]["ground_truths"] = _denorm(arima_data[land]["ground_truths"], mean, std)
            arima_data[land]["samples"]       = _denorm(arima_data[land]["samples"],       mean, std)

        data_tensors[land] = torch.tensor(
            _denorm(data_tensors[land].cpu().numpy(), mean, std),
            dtype=torch.float32,
        )
    ...
    for land in landscapes:
        if land not in norm_stats_map:
            print(f"  [{land}] no norm stats — keeping normalized space")
            continue
        mean, std = norm_stats_map[land]
        nftsf_data[land]["ground_truths"] = _denorm(nftsf_data[land]["ground_truths"], mean, std)
        nftsf_data[land]["samples"]       = _denorm(nftsf_data[land]["samples"],       mean, std)
        if land in arima_data:
            ...
            arima_data[land]["samples"]       = _denorm(arima_data[land]["samples"],       mean, std)
        data_tensors[land] = torch.tensor(
            _denorm(data_tensors[land].cpu().numpy(), mean, std), dtype=torch.float32
        )
